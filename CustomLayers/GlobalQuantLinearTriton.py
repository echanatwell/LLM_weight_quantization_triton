...
def _quantize_global(w_ptr, scale_ptr, out_ptr, n_elements, BLOCK_SIZE: tl.constexpr):
    ...
    w_quant_even = tl.extra.cuda.libdevice.rint(7.0 * w_even * scale).to(tl.int8)
    w_quant_odd = tl.extra.cuda.libdevice.rint(7.0 * w_odd * scale).to(tl.int8)
...
def _matmul_int4_bf16(
    ...
    for start in range(0, tl.cdiv(IN, BLOCK_K)):
        ...
        # w_odd = (w_odd ^ 0x8) - 0x8
        w_even = tl.where(mask_w, w_even - 8, 0.0)
        w_odd = tl.where(mask_w, w_odd - 8, 0.0)
